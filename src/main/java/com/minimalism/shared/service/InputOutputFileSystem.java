...
public class InputOutputFileSystem {
    ...
    public static void createFileSystem(String clientName) throws IOException {
        ...
        createClientTree(clientName.replace(" ", "_"), root);
    ...
    private static void createInputTree(Path beginPath) throws IOException {
        FileSystemConfigHelper fscHelper = FileSystemConfigHelper.getInstance();
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceInputDirectoryName())
                                            .resolve(fscHelper.getServiceInputDataBinDirectoryName()));
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceInputDirectoryName())
                                            .resolve(fscHelper.getServiceInputDataCSVDirectoryName()));
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceInputDirectoryName())
                                            .resolve(fscHelper.getServiceInputDataDefinitionDirectoryName()));
        
                                            
    }

    private static void createOutputTree(Path beginPath) throws IOException {
        FileSystemConfigHelper fscHelper = FileSystemConfigHelper.getInstance();
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceOutputDirectoryName())
                                            .resolve(fscHelper.getServiceOutputDataBinDirectoryName()));
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceOutputDirectoryName())
                                            .resolve(fscHelper.getServiceOutputDataCSVDirectoryName()));
        Files.createDirectories(beginPath.resolve(fscHelper.getServiceOutputDirectoryName())
                                            .resolve(fscHelper.getServiceOutputDataDefinitionDirectoryName()));
        
    }

    private static void createArchiveTree(Path beginPath) throws IOException {
        FileSystemConfigHelper fscHelper = FileSystemConfigHelper.getInstance();
        Path archiveDirectory = Files.createDirectories(beginPath.resolve(fscHelper.getServiceArchiveDirectoryName()));
        createInputTree(archiveDirectory);
        createOutputTree(archiveDirectory);
    }

    private static void createClientTree(String clientName, Path root) throws IOException {
        FileSystemConfigHelper fscHelper = FileSystemConfigHelper.getInstance();
        try {
            Path clientsTree = Files.createDirectories(root.resolve(fscHelper.getServiceClientsDirectory()));
            Path clientTree = Files.createDirectories(clientsTree.resolve(clientName));
            Files.createDirectories(clientTree.resolve(fscHelper.getServiceInstrumentationDirectoryName()));
            Files.createDirectories(clientTree.resolve(fscHelper.getServiceLostAndFoundDirectoryName()));
            createArchiveTree(clientTree);
            createInputTree(clientTree);
            createOutputTree(clientTree);
        } catch (NoSuchPathException e) {
            throw new IOException("Error occured while creating client directory", e);
        }
    }
}
